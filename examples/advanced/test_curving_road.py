#!/usr/bin/env python3
"""
Test runner for curving road scenarios.
...
from pympc.utils import MPCVisualizer, MathUtils
...
def setup_planner(road_path, horizon_length=30):
...
def test_ellipsoid_constraints():
    ...
    # Set up planner
    planner = setup_planner(road_path, horizon_length=30)
    ...
    if solution is not None and planner.is_feasible():
        ...
        visualizer.plot_trajectory(
            trajectory, road_path, obstacles,
            title="Curving Road - Ellipsoid Constraints"
        )
...
def test_gaussian_constraints():
    ...
    if solution is not None and planner.is_feasible():
        ...
        trajectory = solution['states']
        visualizer.plot_trajectory(
            trajectory, road_path,
            title="Curving Road - Gaussian Constraints"
        )
...
def test_scenario_constraints():
    ...
    if solution is not None and planner.is_feasible():
        ...
        visualizer = MPCVisualizer()
        trajectory = solution['states']
        visualizer.plot_scenario_analysis(
            trajectory, scenarios,
            title="Curving Road - Scenario Constraints"
        )
...
def test_combined_constraints():
    ...
    if solution is not None and planner.is_feasible():
        ...
        visualizer.plot_trajectory(
            trajectory, road_path, obstacles,
            title="Curving Road - Combined Constraints"
        )
...
def test_goal_reaching():
    ...
    if solution is not None and planner.is_feasible():
        ...
        visualizer.plot_trajectory(
            trajectory, road_path, obstacles,
            title="Curving Road - Goal Reaching"
        )
