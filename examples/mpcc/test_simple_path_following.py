#!/usr/bin/env python3
"""
Simple path following test that should actually work.

This test uses a very simple approach to path following.
"""
...
class SimplePathFollowingObjective:
    """Simple path following objective that directly minimizes distance to reference points."""
    
    def __init__(self, reference_path, weight=1.0):
        self.reference_path = reference_path
        self.weight = weight
        self.is_active = True
    
    def compute_casadi(self, X, U, opti, **kwargs):
        """Compute simple path following objective."""
        total_cost = 0
        
        for k in range(X.shape[1]):
            position = X[:2, k]  # x, y coordinates
            
            # Get reference point for this time step
            if k < len(self.reference_path):
                ref_point = self.reference_path[k]
                x_ref = ref_point[0]
                y_ref = ref_point[1]
            else:
                # Use last reference point if beyond path length
                ref_point = self.reference_path[-1]
                x_ref = ref_point[0]
                y_ref = ref_point[1]
            
            # Simple distance to reference point
            distance_error = (position[0] - x_ref)**2 + (position[1] - y_ref)**2
            total_cost += self.weight * distance_error
        
        # Add small control penalty
        for k in range(U.shape[1]):
            control_penalty = 0.01 * (U[0, k]**2 + U[1, k]**2)
            total_cost += control_penalty
...
def run_simple_path_following():
    """Run simple path following test."""
    print("=" * 60)
    print("SIMPLE PATH FOLLOWING TEST")
    print("=" * 60)
    
    # Create logger
    logger = MPCLogger()
    logger.log_info("Starting simple path following test")
    ...
    # Create simple path following objective
    objective = SimplePathFollowingObjective(reference_path, weight=10.0)
    logger.log_info("Created simple path following objective")
    ...
    logger.log_info("Created constraints")
    ...
    logger.log_info("Created MPC planner")
    
    # Initialize real-time visualizer
    visualizer = RealtimeVisualizer(
        figsize=(16, 12),
        fps=8,
        save_dir="simple_path_following_plots"
    )
    ...
    logger.log_info(f"Starting simple path following simulation for {max_steps} steps")
    ...
    logger.log_info("Simple path following simulation completed")
    
    # Create animation
    logger.log_info("Creating simple path following animation GIF...")
    gif_path = visualizer.start_animation(
        total_frames=len(trajectory),
        save_gif=True,
        gif_filename="simple_path_following_animation.gif"
    )
    
    logger.log_info(f"Simple path following animation saved: {gif_path}")
    
    # Export data
    data_path = visualizer.export_data("simple_path_following_data.json")
    logger.log_info(f"Simple path following data exported: {data_path}")
    
    # Show final results
    print("\n" + "=" * 60)
    print("SIMPLE PATH FOLLOWING RESULTS")
    print("=" * 60)
    ...
    # Show the animation
    logger.log_info("Displaying simple path following animation...")
...
def main():
    """Main function to run simple path following test."""
    print("🚀 Starting Simple Path Following Test")
    print("This test uses a very simple approach to path following")
    print("The vehicle should follow the curved path: y = 0.1 * x^2")
    print()
    
    try:
        gif_path, data_path, successful_steps = run_simple_path_following()
        
        print("\n" + "=" * 60)
        print("🎉 SIMPLE PATH FOLLOWING TEST COMPLETED!")
        print("=" * 60)
        print(f"✅ Successful steps: {successful_steps}")
        ...
        print(f"\n❌ Simple path following test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
