"""
Fix All Converted Tests

This script fixes the import paths in all converted test files including
...
def fix_converted_test(test_file_path):
    ...
    # For constraint-specific tests, we need to go up more levels
    if "constraints" in str(test_file_path):
        old_path = "project_root = Path(__file__).parent.parent.parent"
        new_path = "project_root = Path(__file__).parent.parent.parent.parent"
        
        if old_path in content:
            content = content.replace(old_path, new_path)
            print(f"   ✅ Fixed constraint test project root path")
...
def fix_all_converted_tests():
    ...
    converted_tests = list(test_dir.rglob("converted_*.py"))
