...
class Contouring:
  ...
  def __init__(self, solver):
    ...
    logger.log(10, "Initializing contouring module")
    self.num_segments = CONFIG["contouring"]["num_segments"]

    # boolean configuration otpions
    self._add_road_constraints = CONFIG["contouring"]["add_road_constraints"]
    self._two_way_road = CONFIG["road"]["two_way"]
    self._dynamic_velocity_reference = CONFIG["contouring"]["dynamic_velocity_reference"]
  ...
  def update(self, state, data, module_data):
    ...
    logger.log(10, "Updating contouring module")

    # update the closest point
    closest_s = None
    _spline = find_closest_point(self, state.getPos(), _closest_segment, closest_s)

    if (module_data.path.get() == None and _spline.get() != None):
  ...
  def set_parameters(self, data, module_data, k):
    ...
    set_spline_parameters(k)

  def set_spline_parameters(self, k):

    for i in range(self.num_segments):

      index = _closest_segment + i

      _spline.get_parameters(index,
                             ax, bx, cx, dx,
                             ay, by, cy, dy)

      start = _spline.get_segment_start(index)


      set_solver_parameter_spline_xa(k, self._solver._params, ax, i)
      set_solver_parameter_spline_xb(k, self._solver._params, bx, i)
      set_solver_parameter_spline_xc(k, self._solver._params, cx, i)
      set_solver_parameter_spline_xd(k, self._solver._params, dx, i)

      set_solver_parameter_spline_ya(k, self._solver._params, ay, i)
      set_solver_parameter_spline_yb(k, self._solver._params, by, i)
      set_solver_parameter_spline_yc(k, self._solver._params, cy, i)
      set_solver_parameter_spline_yd(k, self._solver._params, dy, i)

      # Distance where this spline starts
      set_solver_parameter_spline_start(k, self._solver._params, start, i)
  ...
  def on_data_received(self, data, data_name):
    if data_name == "reference_path":
      ...
      if (data.reference_path.s.empty()):
        _spline = make_shared(RosTools.Spline2D(data.reference_path.x, data.reference_path.y))
      else:
        _spline = make_shared(RosTools.Spline2D(data.reference_path.x, data.reference_path.y, data.reference_path.s))

      if self._add_road_constraints and (not data.left_bound.empty() and not data.right_bound.empty()):

        # Add bounds
        _bound_left = make_unique(RosTools.Spline2D(
            data.left_bound.x,
            data.left_bound.y,
            _spline.getTVector()))
        _bound_right = make_unique(RosTools.Spline2D>(
            data.right_bound.x,
            data.right_bound.y,
            _spline.getTVector()))

        # update the road width
        CONFIG["road"]["width"] = RosTools.distance(_bound_left.get_point(0), _bound_right.get_point(0))
  ...
  def is_objective_reached(self, state, data):
    ...
    return RosTools.distance(self, state.getPos(), _spline.get_point(_spline.parameter_length())) < 1.0
  ...
  def construct_road_constraints_from_centerline(self, data,  module_data):
    ...
    for k in range(_solver.N):
      ...
      vector_2d path_point = _spline.get_point(cur_s)
      vector_2d dpath = _spline.get_orthogonal(cur_s)

      # left HALFSPACE
      A = _spline.get_orthogonal(cur_s)
      ...
      # right HALFSPACE
      A = _spline.get_orthogonal(cur_s) # Vector2d(-path_dy, path_dx) # line is parallel to the spline
  ...
  def construct_road_constraints_from_bounds(self, data, module_data):
    ...
    for k in range(_solver.N):
      ...
      # left
      Al = _bound_left.get_orthogonal(cur_s)
      bl = Al.transpose() * (_bound_left.get_point(cur_s) + Al * data.robot_area[0].radius)
      module_data.static_obstacles[k].emplace_back(-Al, -bl)

      # right HALFSPACE
      Ar = _bound_right.get_orthogonal(cur_s)
      br = Ar.transpose() * (_bound_right.get_point(cur_s) - Ar * data.robot_area[0].radius)
  ...
  def visualize(self, data, module_data):
    ...
    if (CONFIG["debug_visuals"]):
      ...
      visualize_debug_glued_splines(data, module_data)
      visualize_all_spline_indices(data, module_data)
  ...
  def visualize_current_segment(self, data, module_data):
    ...
    cur_point.add_point_marker(_spline.get_point(_spline.get_segment_start(_closest_segment)), 0.0)
  ...
  def visualize_tracked_section(self, data, module_data):
    ...
    i = _closest_segment
    while  i < _closest_segment + _n_segments:

      s_start = _spline.get_segment_start(i)
      s = s_start + 1.0
      while s < _spline.get_segment_start(i + 1):

        if (s > 0):
          line.add_line(_spline.get_point(s - 1.0), _spline.get_point(s))
        s += 1.0
  ...
  def visualize_reference_path(self, data, module_data):
    ...
    visualize_spline(*_spline, _name + "/path", True)
  ...
  def visualize_debug_road_boundary(self, data, module_data):
    ...
    for k in range(_solver.N):
      cur_s = _solver.get_ego_prediction(k, "spline")
      path_point = _spline.get_point(cur_s)

      points.set_color_int(5, 10)
      points.add_point_marker(path_point)

      dpath = _spline.get_orthogonal(cur_s)
  ...
  def visualize_debug_glued_splines(self, data,module_data):

    # Plot how the optimization joins the splines together to debug its internal contouring error computation
    publisher = VISUALS.missing_data(_name + "/glued_spline_points")
    points = publisher.get_new_point_marker("CUBE")
    points.set_scale(0.15, 0.15, 0.15)
    ...
    for k in range(_solver.N):
      ...
      for i in range (self._n_segments):
        ...
        if (index < _spline.num_segments()):
          _spline.get_parameters(index, ax, bx, cx, dx, ay, by, cy, dy)

          start = _spline.get_segment_start(index)
        else:
          # If we are beyond the spline, we should use the last spline
          _spline.get_parameters(_spline.num_segments() - 1, ax, bx, cx, dx, ay, by, cy, dy)

          start = _spline.get_segment_start(_spline.num_segments() - 1)
          ...
          by = 0.
          cy = 0.
          start = _spline.parameter_length()
  ...
  def visualize_all_spline_indices(self, data, module_data):
