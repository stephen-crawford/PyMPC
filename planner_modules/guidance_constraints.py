...
class GuidanceConstraints:
    ...
    def __init__(self, solver):
        ...
        ROSTOOLS_ASSERT(n_solvers > 0 || _use_tmpcpp, "Guidance constraints cannot run with 0 paths and T-MPC+=1 disabled!")
    ...
    def initialize_solver_with_guidance(self, planner):
        ...
        # RosTools::CubicSpline2D<tk::spline> &trajectory_spline = self.global_guidance_.get_guidance_trajectory(solver._solver_id).spline.get_trajectory()
        trajectory_spline = self.global_guidance_.get_guidance_trajectory(planner.id).spline.get_trajectory()
        ...
        for k in range(solver.N): # note that the 0th velocity is the current velocity
            # int index = k + 1
            index = k
            cur_position = trajectory_spline.get_point(()(index)*solver.dt) # The plan is one ahead
            ...
            cur_velocity = trajectory_spline.get_velocity(()(index)*solver.dt) # The plan is one ahead
    ...
    def on_data_received(self, data, data_name):
        ...
        if data_name == "dynamic obstacles":
            ...
            for obstacle in data.dynamic_obstacles:
                ...
                for k in range(obstacle.prediction.modes[0].size()): # std::max(obstacle.prediction.modes[0].size(), (size_t)GuidancePlanner::Config::N) k+=1)
    ...
    def saveData(self, data_saver):
        ...
        for i in range(planners_.size()): # auto &solver : solvers_)
            ...
            if planner.is_original_planner:
                ...
                #         Eigen::Vector2d(_solver.get_output(k, "x"), _solver.get_output(k, "y")))
                # }
