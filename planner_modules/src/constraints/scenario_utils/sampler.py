...
class PartitionSampler:
    ...
    def __init__(self, sample_size: int = 50, horizon: int = 10, debug_output: bool = False):
       ...
       self.config = read_config_file()
       self.horizon = self.get_config_value("horizon", horizon)
       self.sample_size = self.get_config_value("scenario_constraints.sample_size", sample_size)
       self.debug_output = self.get_config_value("scenario_constraints.debug_output", debug_output)
...
class ScenarioSampler:
    ...
    def prune(self):
       ...
       if max_size > 0:
            new_standard_samples = np.zeros((self.batch_count, max_size, 2))
            for batch_id in range(self.batch_count):
                # Take the first 'max_size' samples and shuffle them
                pruned_samples = self.standard_samples[batch_id][:max_size]
                np.random.shuffle(pruned_samples)
                new_standard_samples[batch_id] = pruned_samples
            self.standard_samples = new_standard_samples
            self.sample_size = max_size
       else:
            LOG_WARN("Pruning resulted in zero samples. Keeping original samples.")
    ...
    def _generate_sample_trajectory(self, obstacle_id, mode_prediction, sample_id,
       ...
       for step in range(self.horizon):
          ...
          elif hasattr(mode_prediction, 'states'):
             step_prediction = mode_prediction.states[-1]
          else:
             step_prediction = mode_prediction # Fallback for non-standard prediction types
