...
def generate_casadi_solver(modules, settings, model, skip_solver_generator=False):
    ...
    timestep = settings.get("timenstep", 0.1)
    horizon = settings.get("horizon", 20)

    # Initialize the solver with the appropriate parameters
    solver = CasADiSolver(timestep=timestep, horizon=horizon)
    module_manager = ModuleManager()
    parameter_manager = ParameterManager()
    solver.define_parameters(module_manager, parameter_manager)

    # Set up model dimensions
    solver.nx = model.nx
    solver.nu = model.nu
    solver.nvar = model.get_nvar()
    solver.npar = parameter_manager.parameter_count
...
def generate_osqp_solver(modules, settings, model, skip_solver_generator=False):
    ...
    solver.define_parameters(modules, params)
