"""
Contouring Constraints Test with Static Obstacles

This test uses the IntegrationTestFramework to run a test with:
- Contouring objective
- Contouring constraints
- Linearized constraints (for static obstacle avoidance)
- Several static obstacles placed within the reference path bounds
"""
import sys
import os
import numpy as np
...
def run(dt=0.1, horizon=10, start=(0.0, 0.0), goal=(20.0, 20.0), max_iterations=200, num_obstacles=3):
    """Run Contouring constraints test with static obstacles using linearized constraints."""
    ...
    # Start at 50% to give vehicle time to establish trajectory
    ...
    for i, s_pos in enumerate(obstacle_s_positions):
        ...
        try:
            ...
            print(f"Created static obstacle {i} at s={s_pos:.2f}, position=({x_obs:.2f}, {y_obs:.2f}), offset={offset:.2f}m from centerline")
    ...
    config = TestConfig(
        ...
        test_name="Contouring Constraints with Static Obstacles (Linearized)",
...
def test():
    ...
    # The test demonstrates the linearized constraints working with static obstacles
    return result
