"""
Goal Objective Test with Single Dynamic Obstacle - Ellipsoid Constraints

This test verifies that:
- Goal objective works with Ellipsoid constraints for obstacle avoidance
...
- Constraints: Ellipsoid constraints for obstacle avoidance
"""
import sys
import os
import numpy as np
...
def run(dt=0.1, horizon=10, start=(0.0, 0.0), goal=(10.0, 10.0), max_iterations=200):
    """Run Goal objective test with single dynamic obstacle using Ellipsoid constraints."""
    ...
    # Ellipsoid constraints work with GAUSSIAN prediction type (for uncertainty ellipsoids)
    ...
    # Goal objective with Ellipsoid constraints for obstacle avoidance
    config = TestConfig(
        reference_path=ref_path_points,
        objective_module="goal",
        constraint_modules=["ellipsoid"],  # Ellipsoid constraints for obstacle avoidance
        vehicle_dynamics="unicycle",
        num_obstacles=1,
        obstacle_dynamics=["unicycle"],  # Use unicycle dynamics for obstacle
        test_name="Goal Objective with Single Dynamic Obstacle - Ellipsoid Constraints",
        duration=max_iterations * dt,
        timestep=dt,
        show_predicted_trajectory=True,
        obstacle_configs=[obstacle_config],
        obstacle_prediction_types=["gaussian"],  # Ellipsoid constraints work with GAUSSIAN prediction type
    ...
    print(f"  Behavior: path_intersect (moves back and forth across path)")
    print(f"  Radius: 0.5m")
    print(f"Constraints: Ellipsoid (for obstacle avoidance)")
