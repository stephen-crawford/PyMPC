"""
Road Following MPC Test with Scenario and Contouring Constraints

This test demonstrates a vehicle following a curved road while avoiding
a dynamic obstacle using the full MPC framework with:
- Scenario constraints for obstacle avoidance with predictions
...
class TestRoadFollowingMPC(BaseMPCTest):
    """
    MPC test for vehicle following a curved road while avoiding a dynamic obstacle.
    """
    
    def __init__(self):
        config = TestConfig(
            test_name="test_road_following_mpc",
            description="MPC vehicle follows curved road while avoiding dynamic obstacle",
            timeout=120.0,
            max_iterations=150,
            goal_tolerance=2.0,
    ...
    def setup_test_environment(self):
        ...
        y_path = 4 * np.sin(0.4 * np.pi * t) + 1.5 * np.sin(0.8 * np.pi * t)  # S-curve
    ...
    def setup_mpc_system(self, data):
        ...
        try:
            ...
            # Add scenario constraints for obstacle avoidance with predictions
    ...
    def execute_mpc_iteration(self, planner, data, iteration):
        """Execute one MPC iteration with dynamic obstacle updates and predictions."""
        try:
...
if __name__ == "__main__":
    test = TestRoadFollowingMPC()
    result = test.run_test()
    
    print(f"\n{'='*70}")
    print(f"ROAD FOLLOWING MPC TEST RESULTS")
    print(f"{'='*70}")
    ...
    if result.success:
        print(f"✅ Vehicle successfully followed road and avoided dynamic obstacle using MPC!")
