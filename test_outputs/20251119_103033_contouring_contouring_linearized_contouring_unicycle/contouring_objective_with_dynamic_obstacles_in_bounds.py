"""
Contouring Objective Test with Dynamic Obstacles Moving Within Path Bounds
...
- Linearized constraints are used to avoid the dynamic obstacles
- Vehicle successfully navigates while avoiding obstacles
...
def run(dt=0.1, horizon=10, max_iterations=300, num_obstacles=3):
    """Run Contouring objective test with dynamic obstacles moving within path bounds."""
    ...
    for i, s_pos in enumerate(obstacle_s_positions):
        try:
            ...
            if path_norm > 1e-6:
                ...
                # Set obstacle velocity - move along the path (forward or backward)
                # Alternate directions to create interesting scenarios
                obstacle_speed = 0.8  # Moderate speed
                directions = [1, -1, 1]  # Forward, backward, forward
                direction = directions[i % len(directions)]
                
                velocity = np.array([dx_norm * obstacle_speed * direction, dy_norm * obstacle_speed * direction])
            else:
            ...
            # Create dynamic obstacle (point mass for simplicity)
    ...
    config = TestConfig(
        ...
        constraint_modules=["contouring", "linearized"],  # Contouring for road boundaries + linearized for obstacles
        ...
        test_name="Contouring Objective with Dynamic Obstacles in Bounds",
    ...
    print(f"Constraints: Contouring (for road boundaries) + Linearized (for obstacle avoidance)")
...
def test():
    ...
    # Test with curving reference path and dynamic obstacles moving within bounds
    result = run(max_iterations=300, num_obstacles=3)
    
    # Verify path following, turning, and obstacle avoidance
    assert result, "Vehicle should make progress along the curving path, turn successfully, and avoid all dynamic obstacles"
    return result
