"""
Goal Objective Test with Single Dynamic Obstacle - Linearized Constraints
...
def run(dt=0.1, horizon=10, start=(0.0, 0.0), goal=(10.0, 10.0), max_iterations=200):
    """Run Goal objective test with single dynamic obstacle using linearized constraints."""
    ...
    config = TestConfig(
        ...
        test_name="Goal Objective with Single Dynamic Obstacle - Linearized Constraints",
        duration=max_iterations * dt,
        timestep=dt,
        show_predicted_trajectory=True,
        obstacle_configs=[obstacle_config],
        obstacle_prediction_types=["deterministic"],
