...
def run(dt=0.1, horizon=10, max_iterations=300):
    ...
    num_obstacles = 3  # Number of obstacles to place
    ...
    for i, obstacle_s_position in enumerate(obstacle_s_positions):
        try:
            ...
            obstacle_config.radius = 0.35  # Obstacle radius (smaller for multiple obstacles to allow maneuvering)
            ...
            print(f"Created static obstacle {i} at s={obstacle_s_position:.2f}, position=({x_pos:.2f}, {y_pos:.2f}) on centerline (Gaussian prediction)")
    ...
    config = TestConfig(
        ...
        test_name="Contouring Objective with Contouring Constraints + Multiple Static Obstacles (Gaussian)",
        ...
        obstacle_prediction_types=["gaussian"] * len(obstacle_configs)  # Gaussian constraints use Gaussian predictions
    )
