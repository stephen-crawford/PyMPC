...
from utils.utils import LOG_WARN, LOG_DEBUG
...
class CubicSpline:
    ...
    def get_parameters(self, index):
        ...
        """
        LOG_DEBUG("Trying to get parameters for spline and index, " + str(index))
...
class Spline:
    ...
    def find_closest_point(self, point, range_val=2):
        ...
        LOG_DEBUG("Finding closest point with " + str(self.prev_query_point) + " and " + str(point))
        if self.closest_segment == -1 or (self.prev_query_point is not None and
                                          np.linalg.norm(np.array(self.prev_query_point) - np.array(point))
                                          > 5.0):
        ...
        return self.closest_segment, t_out
...
class FourDimensionalSpline(TwoDimensionalSpline):
    ...
    def __init__(self, x=None, y=None, z=None, w=None, t_vector=None):
        ...
        if z is not None and w is not None:
            ...
            self.z_spline = CubicSpline()
            self.w_spline = CubicSpline()
